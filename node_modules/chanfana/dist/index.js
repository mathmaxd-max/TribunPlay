  ...
  static schema() {
    ...
    return {
      [inst.status]: {
        ...
        ...contentJson({
          success: import_zod4.z.literal(false),
          errors: [innerError]
        })
      }
    };
...
var OpenAPIRoute = class {
  ...
  async execute(...args) {
    ...
    try {
      ...
      if (e instanceof import_zod5.z.ZodError) {
  ...
  async validateRequest(request) {
    ...
    let validationSchema = import_zod5.z.object(rawSchema);
...
var ListEndpoint = class extends OpenAPIRoute {
  ...
  getSchema() {
    ...
    if (this.searchFields) {
      parsedQueryParameters[this.searchFieldName] = import_zod6.z.string().optional().openapi({
        description: `Search by ${this.searchFields.join(", ")}`
      });
    }
    let queryParameters = import_zod6.z.object({
      page: import_zod6.z.number().int().min(1).optional().default(1),
      per_page: import_zod6.z.number().int().min(1).max(100).optional().default(20)
  ...
  async list(filters) {
    ...
    for (const f of filters.filters || []) {
      if (this.searchFields && f.field === this.searchFieldName) {
        const searchCondition = (this.searchFields || []).map((obj) => `UPPER(${obj}) like UPPER(?${conditionsParams.length + 1})`).join(" or ");
