 ...
 */
import { generateLegalActions, decodeAction, unitByteToUnit, decodeCoord, encodeCoord, onBoard, } from './index';
 ...
 */
function getNeighborCid(centerCid, dir) {
    try {
 ...
 */
export function buildGroupedLegalMoves(state) {
    ...
    const enemyOptions = new Map();
    const emptyOptions = new Map();
    const ownPrimaryOptions = new Map();
    const ownSecondaryOptions = new Map();
    // Track which tiles are clickable in Idle
    const enemyTargets = new Set();
    const emptyCenters = new Set();
 ...
 */
export function getClickableTiles(state, uiState, grouped) {
    switch (uiState.type) {
        case 'idle':
            return [
 ...
 */
export function getEnemyOptions(targetCid, grouped) {
    return grouped.enemyOptions.get(targetCid) || [];
}
/**
 * Get options for Own.Primary state
 */
export function getOwnPrimaryOptions(originCid, targetCid, grouped) {
    const primaryOpts = grouped.ownPrimaryOptions.get(originCid);
    if (!primaryOpts)
        return [];
    if (targetCid === null) {
 ...
 */
export function getOwnPrimaryHighlightedTiles(originCid, grouped) {
    const primaryOpts = grouped.ownPrimaryOptions.get(originCid);
    if (!primaryOpts)
        return [];
 ...
 */
export function getOwnSecondaryPendingAction(originCid, allocations, grouped, state) {
    ...
    for (let i = 0; i < 6; i++) {
        if (allocations[i] > 7)
            return null; // SPLIT can't encode 8
 ...
 */
export function getAllowedAllocationValues(originCid, neighborDir, allocations, state) {
    const originUnit = unitByteToUnit(state.board[originCid]);
    if (!originUnit)
        return [0];
    const H0 = originUnit.p;
    const remAfterOthers = H0 - allocations.reduce((sum, val, idx) => idx !== neighborDir ? sum + val : sum, 0);
 ...
 */
export function getEmptyStateOptions(centerCid, donors, // cid -> displayed primary (Hdisp)
state, grouped) {
    const emptyOpts = grouped.emptyOptions.get(centerCid);
    if (!emptyOpts)
        return [];
    ...
    if (participatingDonors.length === 2) {
        ...
        const dirs = participatingDonors.map(([cid]) => {
            for (let dir = 0; dir < 6; dir++) {
                if (getNeighborCid(centerCid, dir) === cid)
                    return dir;
            }
            return -1;
        }).filter(d => d >= 0);
        if (dirs.length === 3) {
            ...
            if (isSym3Config1 || isSym3Config2) {
                const config = isSym3Config1 ? 1 : 2;
 ...
 */
export function isActionLegal(action, grouped) {
 ...
 */
export function getEmptyStateDonors(centerCid, state) {
 ...
 */
export function getTileClickState(cid, state, grouped) {
    const unit = unitByteToUnit(state.board[cid]);
    if (!unit) {
 ...
 */
export function getValidDonationValues(donorCid, state) {
    ...
    for (let donate = 1; donate <= maxDonate; donate++) {
        ...
        if (!validRemainders.has(remaining))
            continue;
        if (remaining > 0 && unit.s > 0 && (remaining > 4 || 2 * remaining < unit.s))
            continue;
        allowed.push(donate);
    }
    return allowed;
}
//# sourceMappingURL=ui-backend.js.map